...
def learn_variable_unigram(data, verbose=True):
    unigramDataPerplex={}
    ...
    for i in range(10,101,10):
        ...
        takingpoint = int(length_data_train*i/100)
        datataken = data.train[:takingpoint]
        unigram.fit_corpus(datataken)
        temp=[]
        temp.append(unigram.perplexity(data.train))
        temp.append(unigram.perplexity(data.dev))
        temp.append(unigram.perplexity(data.test))
        unigramDataPerplex[i] = temp
    print(unigramDataPerplex)
    textfile=open("perplexities_unigram_data.txt","a")
    textfile.write(json.dumps(unigramDataPerplex))


def learn_trigram(data, delta = 0.001, verbose=True):
    from lm import Trigram
    trigram = Trigram(backoff=0.000001, delta=0.001)
...
def learn_unigram_adaption(data1,data2,data3, verbose=True):

    from lm import Unigram
    unigram = Unigram()
    # unigram.fit_corpus(data3.train+data3.train[:int(0*len(data3.train))  ])
    unigram.fit_corpus(data3.train)
    if verbose:
        print("vocab:", len(unigram.vocab()))
        # evaluate on train, test, and dev
        print("train:", unigram.perplexity(data3.train))
        print("dev  :", unigram.perplexity(data3.dev))
        print("test :", unigram.perplexity(data3.test))
        from generator import Sampler
        sampler = Sampler(unigram)
        print("sample 1: ", " ".join(str(x) for x in sampler.sample_sentence([])))
        print("sample 2: ", " ".join(str(x) for x in sampler.sample_sentence([])))
...
if __name__ == "__main__":
    ...
    data1 = read_texts("data/corpora.tar.gz", dnames[0])
    data2 = read_texts("data/corpora.tar.gz", dnames[1])
    data3 = read_texts("data/corpora.tar.gz", dnames[2])
    model = learn_unigram_adaption(data1,data2, data3)
    # for dname in dnames:

    #     model = learn_unigram(data)
    #     # model = learn_variable_unigram(data)
